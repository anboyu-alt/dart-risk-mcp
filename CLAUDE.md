...
- **응답 필드는 실제로 떠서 확인한다**: 존재하지 않는 필드를 읽어 그 자리가 영원히 비어 있던 결함이 2026-08-26까지 **세 건** 나왔다 — `fetch_debt_balance`(도구 하나가 통째로 죽음, #315) · `corp_cls_nm`(「법인구분」이 모든 회사에서 「-」, #327) · `reprt_nm`(「5% 대량보유자」의 **이름 자리가 늘 비어 있었다**). 셋 다 테스트는 통과하고 있었다(가짜 응답이 같은 거짓 필드명을 썼거나, 그 줄을 아무도 안 봤다). 새 필드를 읽기 전에 그 엔드포인트를 한 번 호출해 `list[0].keys()`를 확인한다. 42개 엔드포인트의 실제 키 383종은 `scripts/`가 아니라 그때그때 뜬다 — 캐시하지 않는 이유는 DART가 필드를 추가할 수 있어서다.
