...
| 제목으로 발화 불가한 신호 17종 (v1.12.3 실측) | ⚠ | 1년 코퍼스(고유 공시 201,361건)에서 **모든 키워드가 0건**인 신호: `CB_REPAY`·`CB_ROLLOVER`·`CB_BUYBACK`·`TREASURY_EB`·`CAPITAL_RED`·`GAMJA_MERGE`·`RIGHTS_UNDER`·`CIRCULAR`·`ACTIVIST`·`MEETING_VIOL`·`RELATED_PARTY`·`BUYBACK_NEG`·`DISTRESS_MA`·`ASSET_TRANSFER`·`REVENUE_IRREG`·`CONTINGENT`·`ASSET_SPIRAL`. 일부는 구조화 경로 보유(`CB_ROLLOVER`→`detect_debt_rollover`)지만 나머지는 제목이 유일 경로인데 막혀 있다 — `search_market_disclosures("asset_transfer")`는 구성 3종 중 2종이 사장. **계절성 가설은 기각됐다**(90일 0건 122개 중 365일에도 114개가 0건, `AUDIT`의 한정의견·감사인교체는 1년 내내 0건 — 감사의견은 제목이 아니라 사업보고서 본문에 있고 그 경로는 `fetch_audit_opinion_history`가 이미 담당). 실제 DART 표기와 미해결 갭 목록은 `docs/superpowers/specs/2026-08-22-signal-keyword-audit.md` 3.5절 |
...
| `fund_diversion_chain` (복합 패턴 발화) | **원문 확인** | `fetch_acquisition_detail`(`parse_acquisition_detail`) → `classify_outflow_relation` → `_fund_diversion_gate` | 요구 신호가 1.1+5.8 둘뿐이라 겹침 2개면 곧 전부 일치다 — `ACQ_REVIEW` 재현율 수정 후 1년 기준 **142개사**에서 발화해 제목만으로는 정상 사업확장 M&A와 구분되지 않는다. 취득 대상이 **계열·특수관계로 확인될 때만** 패턴을 표시하고, 그 외에는 확인된 사실(대상 실명·관계·금액·자기자본 대비)만 블록으로 남긴다. 종속회사는 통과시키지 않는다(모회사의 자회사 지분 확대는 정상적 지배구조 정리 — `capital_backflow`와 같은 판단). **DS005를 쓰지 않는다**: 「타법인주식및출자증권취득결정」(자율공시, 4배 흔함)은 `resolve_decision_type`이 빈 값이고, 법정 「…양수결정」조차 DS005가 '구조화 데이터 없음'을 반환하는 사례가 실측됐다(KR모터스 20251001·코아스 20250904). 원문은 두 서식 모두 구조가 일정해 25개사 32건 표본에서 **32/32(100%)** 파싱 성공 |
