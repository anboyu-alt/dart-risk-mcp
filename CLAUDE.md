...
**1년 창이 확정해 준 것**: 90일에서 "0건이지만 창 밖일 수도"였던 키워드
**9종**이 1년 전수에서도 0건으로 확정됐다 — `AUDIT`의 한정의견·부적정의견·
계속기업불확실성·감사범위제한·감사인교체, `GOING_CONCERN`의 계속기업가정
불확실·계속기업불확실, `INSOLVENCY`의 어음부도·의도적부도(공백 변형까지
정규식으로 재확인). 그래서 `AUDIT`은 사실상 **"의견거절" 하나로만**(15건),
`GOING_CONCERN`은 **"회생절차"(216)·"파산절차"(7)로만** 발화한다 — 이름은
"계속기업불확실"인데 계속기업 관련 표기는 DART 제목에 아예 없다. 어순·공백
때문에 새는 재현율 갭도 함께 측정했다(「반기검토**의견부적정**」 13건은 어순이
반대, 「감사**의견 거절**」 17건은 공백). 정리는 별도 판단 — 근거:
`docs/superpowers/specs/2026-08-22-corpus-refresh.md` `tests/test_corpus_invariants.py`가
