...
**앱이 `display_name`을 서버 이름으로 쓴다 — 확정(2026-09-14).** 근거 둘:
① 데스크톱 앱의 MCP 로그 파일 이름이 곧 서버 이름인데 실물이
`%APPDATA%\Claude\logs\mcp-server-DART 리스크 분석 (불공정거래 위험 모니터링).log`
다(같은 폴더의 `mcp-server-korean-dart.log`와 나란히 있다) ② 앱이
`%APPDATA%\Claude\extensions-installations.json`에 **manifest 전체를 캐시**하고
거기 `…/manifest/display_name`이 그 값으로 들어 있다.

⚠ **그래서 `.mcpb` 재설치는 두 곳을 함께 갱신해야 한다** — 설치 폴더의
`manifest.json`만 손으로 바꾸면 캐시가 옛 이름을 들고 있고, 앱이 실행 중이면
종료할 때 그 캐시를 도로 써서 수정이 날아간다. **앱 UI로 정식 재설치**해야
한다. 재설치가 먹었는지는 재시작 뒤 `logs\mcp-server-dart-risk.log`가 새로
생기는지로 본다.
