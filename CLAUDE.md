...
| `CB_BW→1.1/1.5/1.6` | **보류** — 1.1(리픽싱)은 「전환사채권발행결정」에도 붙는데, `fund_diversion_chain`(1년 **132장**, 최다 발화)의 요구 신호라 파급이 크다. 패턴은 1.1을 "CB 조달"의 뜻으로 쓰고 taxonomy 이름은 "Refixing"이라 **이름과 용법이 어긋난다**. taxonomy 신설·개명은 카탈로그 277건 매핑과 뷰어에 연쇄로 걸리므로 별도 판단으로 남긴다 |
