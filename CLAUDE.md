...
⚠ **확정하지 못한 것 하나** — 옛 접두어는 **29자**인데 설치본
`display_name`은 **27자**였다(설치본 manifest를 직접 열어 대조). 2자가 어디서
오는지 설명하지 못했다. 즉 앱이 그 필드를 접두어로 쓴다는 것은 **추정이고,
`.mcpb`를 다시 만들어 설치해 접두어가 `mcp__dart-risk__…`로 바뀌는지 봐야
확정된다.** 안 바뀌면 출처가 다른 곳이다.
