...
> ⚠ **잘리는 쪽이 최근이었다(2026-09-12)**. 위 라운드는 「몇 건을 잘랐나」를 고쳤고 **「어느 쪽을 잘랐나」는 보지 않았다**. `detect_capital_churn`의 `events`는 `rcept_dt` **오름차순**인데(`events.sort(key=lambda e: e["rcept_dt"])`) 두 곳이 `[:N]`으로 잘라 **가장 오래된 것만 남기고 최근을 버렸다** — 불공정거래 모니터링에서 가장 중요한 구간이 사라진다. 라이브: `track_capital_structure` 제이스코홀딩스 3년 조회의 시계열 마지막 줄이 **2025-06-27**이라 그 뒤 1년이 통째로 빠졌다(수정 후 **2026-07-14**). `analyze_company_risk`의 「자본 변동 타임라인」은 12개월 창이라 범위가 좁지만 **15개사 중 7곳이 상한 10건을 넘는다**(코아스 21 · 유티아이 21 · CSA 코스믹 15 · HLB 14 · 진원생명과학 14 · KR모터스 12 · 오르비텍 12) — 코아스는 11건이 잘렸고 그게 전부 최근이었다. **최근 N건을 고르되 표시는 시간순**을 지킨다(시계열은 흐름이라 뒤집으면 안 된다). 문구도 「총 N건 중 **최근** M건 표시 · **앞선** K건 생략」으로 방향을 밝힌다. CLAUDE.md의 `FETCH_TRUNCATED` 기록과 같은 계열이다 — 그때는 list.json이 최신순이라 오래된 쪽이 잘렸고 여기서는 정렬이 반대라 최근이 잘렸다. 공통점은 **어느 쪽이 잘리는지 화면이 말하지 않았다**는 것. ⚠ 기계 검사로는 못 찾는다 — 두 곳 다 「총 N건 중 30건 표시」라 **건수는 이미 밝히고 있었다**. 방향은 정렬을 읽어야 안다. `tests/test_timeline_truncation_direction.py`가 고정.
