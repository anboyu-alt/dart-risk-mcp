...
| 패턴의 순서·간격 축(`timeline_months`) | ❌ 도입 안 함 | 12개사×3년 실측 — 순서 일치율 **64%**(무작위 50%), `timeline_months` 안에 들어온 관측 **49%**(실제 기간 중앙값 9개월·최대 35개월). 제약으로 쓰면 진짜 사례를 떨어뜨린다. 원인은 이 도구가 사건 발생 순서가 아니라 **공시 접수일**을 보기 때문. 근거: `docs/superpowers/specs/2026-08-21-pattern-sequence-measurement.md` |
...
라이브 효과(2026-08-21 실측): 아틀라스링크 `자금 역류` 2/2는 그대로 발화(창 20260709),
제이스코홀딩스는 6개 → 4개로 정리(`감사의견 내부자 덤프`·`허위 신사업 주가부양`이
연도를 걸친 조합이라 탈락, `무자본 M&A` 4/6→3/6), 셀트리온·삼성전자는 전후 모두 0건.
