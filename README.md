...
도움말이 출력되면 설치 성공입니다. 이제 사용 중인 AI 클라이언트에 등록만 하면 됩니다(아래 방법 A~D 중 본인 환경에 맞는 것 하나만 골라 진행).

> **선택 — uv 사용자라면:**
> uv가 이미 깔려 있다면 위 `pip install`을 건너뛰고, 아래 JSON 설정에서 `"command": "dart-risk-mcp"` 대신 `"command": "uvx", "args": ["dart-risk-mcp"]`을 사용하면 됩니다(별도 설치 없이 임시 환경에서 자동 실행). uv 설치는 Windows: `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"` · macOS/Linux: `curl -LsSf https://astral.sh/uv/install.sh | sh`.
>
...
위 명령은 0단계에서 `pip install dart-risk-mcp`을 마쳤다고 가정합니다. uv 사용자는 `-- dart-risk-mcp` 대신 `-- uvx dart-risk-mcp`을 사용하면 사전 pip 설치 없이도 작동합니다.
