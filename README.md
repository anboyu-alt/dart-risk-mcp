...
> **버전 고정이 필요하다면:** `uvx dart-risk-mcp` 대신 `uvx dart-risk-mcp==1.0.1`을 사용하세요. PyPI에 PEP 440 형식으로 받아들여집니다. 개발판(master HEAD)이 필요한 경우는 `uvx --from git+https://github.com/anboyu-alt/dart-risk-mcp.git dart-risk-mcp`.
...
```bash
uvx --refresh dart-risk-mcp --help
```

`--refresh` 플래그가 해당 패키지의 캐시만 무효화하고 PyPI에서 최신 코드를 다시 내려받습니다. 명령어가 도움말을 출력하면 정상입니다.
