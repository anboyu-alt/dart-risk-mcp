...
**검증 방식**: 유가증권·코스닥 6개사 × 25개 도구의 실측 골드 출력 300건 + 테스트 3,000여 개가 저장소에 있으며, 모든 변경은 이 실측 출력과 대조해 회귀 검증됩니다. 라이브로 재현된 적 없는 신호는 [CLAUDE.md](CLAUDE.md)의 검증 매트릭스에 ⚠로 정직하게 표기합니다.
