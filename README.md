...
>
> 🔎 **설치 없이, 키 없이 바로** — [리스크 뷰어](https://dart-risk-mcp.vercel.app)에서 종목명만 넣으면 하루 **5곳까지 키 없이** 스캔됩니다. 아래 [리스크 뷰어](#-설치-없이-먼저-써-보기--리스크-뷰어) 절 참고.
...
**그리고 신호를 찾지 않을 때도 씁니다.** 도구 34개 중 위험 분석 전용은 일부이고, 나머지는 조회·확인용입니다 — 공시 원문을 섹션·페이지로 읽고, 주석 본문을 낱말로 검색하며, 재무제표 전체 계정을 **감사보고서 원문의 계정명과 대조**해 냅니다(DART API가 주는 이름은 XBRL 표준 태그라 회사가 쓴 말과 다를 때가 있습니다 — 「이익잉여금」이 원문에는 「결손금」인 식). **비상장 외부감사대상 법인**의 재무제표·주석도 감사보고서 원문에서 읽고, 최대 20개사 × 10개 연도를 나란히 비교합니다. 「이 회사 매출원가가 얼마야?」 같은 평범한 질문에도 답합니다.

---

## 🚗 그런데 "MCP"가 뭐예요?

한마디로 **AI에게 바깥 세상과 연결되는 "옵션 장치"를 달아 주는 표준 규격**입니다(Model Context Protocol). 자동차에 빗대면 쉽습니다.

- **그냥 AI 채팅** = 깡통차. 잘 달리지만 아는 건 배운 것뿐이라, 오늘 올라온 공시나 내 데이터는 보지 못합니다.
- **'프로젝트' 같은 기본 기능**을 쓰는 정도 = 에어컨 옵션 하나 단 셈. 조금 편해지죠.
- **MCP를 붙이면** = 내비게이션을 단 것과 같습니다. AI가 바깥(여기선 DART 전자공시)에 **직접 접속**해 길을 찾아 줍니다. 잘 엮어 쓰면 여러 도구를 알아서 조합하는 **자율주행급**까지 갑니다.
...
## 🔎 설치 없이 먼저 써 보기 — 리스크 뷰어

**[dart-risk-mcp.vercel.app](https://dart-risk-mcp.vercel.app)** — 아무것도 설치하지 않고, **키도 없이** 종목명만 넣으면 됩니다.

MCP 설치는 터미널이나 Claude Desktop이 필요합니다. 뷰어는 브라우저만 있으면 되니, **이 도구가 무엇을 보는지 먼저 눈으로 확인**하고 설치를 결정하세요.

![리스크 뷰어 결과 대시보드 — 코아스 1년 스캔. 상단에 전체 공시·신호 감지·자본 이벤트·정정 비율, 아래에 가장 무거운 신호와 먼저 볼 공시, 우측에 카테고리 분포와 신호 체인](docs/images/viewer/viewer-dashboard.png)

*종목명을 넣으면 나오는 첫 화면. 왼쪽은 이 기간 가장 무거운 신호와 「먼저 볼 공시」, 오른쪽은 신호가 어느 유형에 몰렸는지와 시간순 연결(신호 체인)입니다.*

### 어떻게 시작하나

1. 링크를 엽니다 → 종목명(예: `셀트리온`)이나 6자리 종목코드를 입력
2. 최근 1년 공시를 **그 자리에서 실시간 수집**해 분석합니다(수십 초)
3. 끝. 회원가입도, 설치도, 키 입력도 없습니다

**하루 5곳까지 키 없이** 조회됩니다. 더 보시려면 본인의 무료 DART API 키를 넣으면 됩니다 — [발급](https://opendart.fss.or.kr)은 증빙서류 없이 즉시 되고 하루 20,000건입니다. 입력한 키는 **브라우저에만** 저장되고 서버에 남지 않습니다.

### 화면 다섯 개

| 화면 | 무엇을 보나 |
|---|---|
| **스캔 입력** | 종목 검색 · 최근 스캔 기록 · 즐겨찾기 |
| **공시 피드** | 수집한 공시 전체를 날짜순으로. 카테고리·신호·정정으로 필터 |
| **공시 상세** | 한 건의 신호·해설·원문 열람(페이지에서 바로) |
| **결과 대시보드** | 아래 패널 14개 |
| **임원 겸직 비교** | 2~5개사의 임원 명단을 다년으로 겹쳐 **공통 인물**을 찾습니다 |

### 대시보드가 보여주는 것

관측 타임라인 · 신호 해설 · 최대주주 변경 상세 · 손익구조 급변 · 재무 핵심 · 회전율 추세 · 메자닌(CB·BW·EB) 현황 · 주식 수 변동(희석) · 배당과 순이익 · 조달 자금의 행방 · 임원·주요주주 소유보고 시계열 · 감사의견 3개년 · 채무증권 미상환 잔액.

수치는 **표**로 나오고 증감에는 ▲▼가 붙습니다. 색은 그 지표에서 **통상 개선으로 읽히는 쪽**을 가리키며, 좋고 나쁨이 갈리는 지표는 색 없이 둡니다.

![리스크 뷰어 재무 패널 — 계정별 전기·당기·증감 표, 부채비율·유동비율 3기간 추이, 회전율 5종과 현금전환주기](docs/images/viewer/viewer-tables.png)

*같은 화면을 내려가면 나오는 재무 패널. 같은 ▼라도 **부채비율은 초록**(내려가는 쪽이 통상 개선), **유동비율은 빨강**이고, 방향에 좋고 나쁨이 없는 **매입채무회전율은 색이 없습니다**. 값이 없는 칸은 비워 두지 않고 `―`로 적습니다.*

### 뷰어도 점수를 매기지 않습니다

MCP 도구와 같은 원칙입니다 — 등급·점수·매수 추천이 없고, 공시라는 공개 기록의 **사실 표기와 원문 링크**만 냅니다. 모든 행에서 DART 원문으로 바로 갈 수 있습니다.

> ⚠ 뷰어는 **웹 축약판**입니다. 위험 신호 42종·복합 패턴 11종을 1년 창에서 보여주지만, 다년 조회·비상장 감사보고서·주석 낱말 검색·원문 계정명 대조 같은 것은 MCP 도구(아래 34개)에 있습니다. 뷰어에서 흥미로운 회사를 찾고, 깊게 파는 것은 MCP로 — 그 순서를 권합니다.
...
- `KRX_API_KEY`(선택) — [openapi.krx.co.kr](https://openapi.krx.co.kr)에서 발급(유가증권·코스닥 일별매매정보 활용 신청 필요). `track_market_reaction`(공시 전후 시세·거래량 대조) 전용이며, 없어도 나머지 도구는 그대로 작동합니다. `env`에 `"KRX_API_KEY": "발급받은_키"`를 함께 넣으세요.

</details>

> 💡 **설치가 부담스럽다면** — 아무것도 깔지 않고 **키도 없이** 하루 5곳까지 써 보는 [리스크 뷰어](#-설치-없이-먼저-써-보기--리스크-뷰어)가 있습니다.
...
| 축 | 내용 |
|---|---|
| **신호 탐지** | 위험 상관이 높은 공시 유형 **42종**(CB/BW, 무상감자, 최대주주 변경, 감사의견 이슈, 조회공시, 회생절차 …)을 8개 카테고리로 자동 표시. 금감원·금융위 공개 적발 사례 기반 키워드 |
...
> 🔀 **공시 도구를 여러 개 쓰고 있다면** — 이 서버의 이름은 **`dart-risk`** 입니다.
> 「**dart-risk로** 셀트리온 봐줘」처럼 앞에 붙이면 이쪽으로 옵니다.
> `dartrisk` · `다트리스크` · `다트 리스크` 라고 불러도 같은 곳을 가리킵니다(서버가 스스로 알립니다).
> 다른 DART 도구를 밀어내지는 않습니다 — 재무제표·원문 조회는 익숙한 도구를 쓰시면 됩니다.
...
**검증 방식**: 유가증권·코스닥 10개사 × 32개 도구의 실측 골드 출력 304건 + 테스트 5,700여 개가 저장소에 있으며, 모든 변경은 이 실측 출력과 대조해 회귀 검증됩니다. 라이브로 재현된 적 없는 신호는 [CLAUDE.md](CLAUDE.md)의 검증 매트릭스에 ⚠로 정직하게 표기합니다.
...
- **점수·등급 없음** — "위험도 82점", "고위험" 같은 표기를 일절 하지 않습니다. 정량화된 낙인은 그 자체로 판정이 되고 투자 권유로 오독됩니다. 이 원칙은 자동 회귀 테스트(`tests/test_golden_output_hygiene.py`)가 기계적으로 지킵니다.
  - 한정 예외: **공개 리스크 뷰어**는 신호에 2단계 관찰 우선순위(주의/참고)를 배지로 표시합니다. 이는 "무엇부터 볼 것인가"의 정렬 표시이지 위험 판정·점수·등급이 아니며, 뷰어 화면에 같은 취지의 면책이 함께 표기됩니다. MCP 도구 출력에는 적용되지 않습니다.
...
| 시세로 판정·예측 | `track_market_reaction`도 등락률·거래량 배수·회전율을 사실로만 표기하고 "급등"·"급락"·"이상" 같은 판정 어휘를 쓰지 않습니다 |
...
- **행위자 연결망 시각화** — 반복 등장 행위자와 회사의 연결망을 보여주는 인터랙티브 화면을 별도로 운영 중입니다([열기](https://dart-risker.pages.dev/)). 화면에는 실명이 포함되며, 표기는 공시에 적힌 명단을 그대로 옮긴 **사실 표기이지 판정이 아니고** 동명이인·동명 법인은 미확인입니다.
...
- 아키텍처·34개 도구 상세·DART 엔드포인트 맵·신호 추가 방법: **[CLAUDE.md](CLAUDE.md)** (개발자 가이드)
