...
> ⚠ 뷰어는 **웹 축약판**입니다. 위험 신호 42종·복합 패턴 11종을 1년 창에서 보여주지만, 다년 조회·비상장 감사보고서·주석 낱말 검색·원문 계정명 대조 같은 것은 MCP 도구(아래 33개)에 있습니다. 뷰어에서 흥미로운 회사를 찾고, 깊게 파는 것은 MCP로 — 그 순서를 권합니다.
...
**검증 방식**: 유가증권·코스닥 10개사 × 31개 도구의 실측 골드 출력 304건 + 테스트 5,700여 개가 저장소에 있으며, 모든 변경은 이 실측 출력과 대조해 회귀 검증됩니다. 라이브로 재현된 적 없는 신호는 [CLAUDE.md](CLAUDE.md)의 검증 매트릭스에 ⚠로 정직하게 표기합니다.
...
- 아키텍처·33개 도구 상세·DART 엔드포인트 맵·신호 추가 방법: **[CLAUDE.md](CLAUDE.md)** (개발자 가이드)
