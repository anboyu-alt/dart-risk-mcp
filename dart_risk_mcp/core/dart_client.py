...
# ⚠ 「해당없음」(사항 없이)이 빠져 있었다 — 뷰어가 그 값을 「집행 차이 사유
# 보고 있음」으로 바꿔 **뜻이 정반대로** 나왔다(NAVER 실측). 드물다 —
# 14개사 510행에서 2행(0.4%)이지만 걸리면 화면이 반대를 말한다.
# 공백형 「해당 없음」은 표본에 없었으나 바로 위 「해당사항없음/해당사항 없음」이
# 이미 두 표기를 함께 두고 있어 같은 관례로 넣는다.
...
#
# 국내 시장 상장 여부만 본다(해외 증시 상장은 판별 대상이 아니다).
# corpCode.xml은 상장·비상장을 모두 담은 DART 공시대상 법인 명부(11만여 건)라,
# 여기서 못 찾는다는 것은 **국내 상장사가 아니라는 강한 근거**다.
#
# ⚠ 여기 「조합·펀드·투자신탁·리츠 같은 이름은 이름만 보고 unlisted로
# 본다」는 `_NON_CORP_MARKERS` 목록이 **정의만 된 채** 있었다(2026-08-27
# 제거). 쓰이지 않은 것이 다행이다 — 명부 실측에서 **그 마커를 가진 채
# 진짜 상장 종목코드를 달고 있는 법인이 40곳**이다.
#
...
# 없는 패턴이 떴을 것이다. 이름은 실체를 말해 주지 않는다 — 명부를 본다.
...
_DISPOSAL_AMOUNT_RE = re.compile(
    # 양수(사는 것) 표기도 함께 본다 — 이 파서를 쓰는 블록이 보는 것은
    # 자금이 나가는 방향이고, 양수야말로 돈이 나가는 쪽이다.
    r"(?:처분금액|처분가액|양도금액|양도가액|양수금액|양수가액|취득금액|취득가액)"
    r"\s*\(?원\)?\s*([\d,]+)"
)
...
def parse_asset_disposal_detail(text: str) -> dict:
    ...
    if _is_amended_document(text):
        return out
    # ⚠ 게이트에 **양수(사는 것)**를 넣지 않아 그 서식을 통째로 거절하고 있었다
    # (2026-08-28 실측). 상대방 정규식 #0(「거래상대방 회사명(성명)」)은 양수
    # 문서를 이미 읽을 수 있는데도 여기서 막혔다 — 12건 표본에서 **양도 5건은
    # 5건 다 읽고 양수 6건은 0건**이었다. 이 파서를 쓰는
    # `_confirm_outflow_counterparties`가 보는 것은 **자금이 나가는 방향**이고,
    # 양수야말로 돈이 나가는 쪽이다. 가장 알고 싶은 방향을 못 읽고 있었다.
    #
    # 라이브: 아틀라스링크 20260810000747 「유형자산양수결정」 원문에
    # 「6. 거래상대방 회사명(성명) 정은산업 주식회사 … 216억」이 있는데 화면은
    # 「(미확인)」이었다(뷰어는 같은 원문에서 읽어 두 화면이 서로 달랐다).
...
_FS_ALIASES = {
    ...
    # 2026-08-25 실측(39개사 재무제표 전수): 별칭 둘로는 **8곳에서 매출채권을
    # 못 찾았다**(DSRI 27/35). `_pick_account`는 **정확 일치**라 표기가 다르면
    # 무조건 실패하고, 그러면 `AR_SURGE` 플래그와 DSRI가 **조용히 사라진다** —
    # 그 회사에서는 영원히 뜨지 않는다.
    #
