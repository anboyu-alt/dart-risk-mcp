...
TAXONOMY = {
    ...
    "1.1": {
        ...
        "keywords": [
            "리픽싱",
            "전환가액조정",
            # DART 실제 제목은 조사가 낀 「전환가액**의**조정」이다(1년 338건).
            # 개념어는 검색어가 아니지만(#297) `test_mapping_evidence_sweep`이
            # 이걸로 매핑 근거를 재기 때문에, 조사 하나 차이로 근거 있는 매핑이
            # 0%로 보였다 — CLAUDE.md가 CB_BW→1.1을 「보류」로 남긴 이유다.
            # 1년 코퍼스 전수에서 이런 근사 불일치는 **이 한 건뿐**이다.
            "전환가액의조정",
            "전환가액인하",
            "전환가격인하",
            "전환가격 인하",
    ...
    "5.5": {
        ...
        "severity": "HIGH",
        "crisis_timeline_months": 12,
        # ⚠ 좁힘(2026-09-02, 위험 목록 14번). 옛 개념어 「분할」·「분할결정」은
        #   유형 이름의 조건(**비대칭**)을 뺀 낱말이라 정상 공시를 삼켰다 —
        #   1년 전수에서 「분할」 405건 중 **366건**이 「주권매매거래정지(주식의
        #   병합, 분할 등 전자등록 변경)」류였고 「분할결정」 75건 중 41건이
        #   **주식**분할결정(액면분할)이었다. v1.12.3이 신호 키워드를
        #   「회사분할결정」으로 좁힐 때 이쪽은 따라오지 않은 문서 드리프트다.
        #   실측: 「회사분할결정」은 46건 전부가 DEMERGER 관찰 제목이고 딴 데 0건이며,
        #   「주식분할결정」에는 포함되지 않는다.
        "keywords": [
            "회사분할결정",
            "비대칭분할",
    ...
    "5.6": {
        ...
        "severity": "OBSERVATION",
        "crisis_timeline_months": 12,
        # ⚠ 비움(2026-09-02, 위험 목록 14번). 이 유형의 조건은 「**적자 중**」인데
        #   그것은 공시 제목에 원리적으로 나타나지 않는다 — 옛 개념어
        #   「현금ㆍ현물배당결정」은 1년 전수 1,770건을 잡았고 그 **전부가 정상
        #   배당**이었다. 소유 신호 `DIVIDEND_DRAIN`은 제목이 아니라
        #   `detect_dividend_drain`(alotMatter의 연도별 순이익 × 현금배당)이 만드는
        #   합성 플래그라 개념어가 설 자리가 없다 — `8.5` 계열(무점수 관찰 유형)이
        #   같은 이유로 비어 있는 것과 같다.
        "keywords": [],
        "red_flags": [
            "당기순이익 적자 + 현금배당 양수",
    ...
    "8.1": {
        ...
        "severity": "CRITICAL",
        "crisis_timeline_months": 6,
        # ⚠ 좁힘(2026-09-02, 위험 목록 14번) — 「회생」 **하나만** 뺐다.
        #   「회생」은 1년 전수 118건이 전부 정상적인 회생절차개시신청이고, 그 공시는
        #   `GOING_CONCERN`(→ **8.4**)이 담당한다 — 8.1의 낱말이 아니다.
        #   ⚠ **「자본잠식」·「부도」는 남긴다.** 처음엔 셋 다 뺐다가 되돌렸다 —
        #   8.1의 소유 신호는 `EMBEZZLE` 하나가 아니라 **`INSOLVENCY`·`FUND_DIVERSION`
        #   까지 셋**이고, 자본잠식·부도발생 공시는 `INSOLVENCY`가 8.1로 매핑하는
        #   **관찰 가능한 얼굴**이다. 첫 측정이 첫 소유자(`EMBEZZLE`)만 보고
        #   「셋 다 관찰 제목에 0건」이라 적었는데, 그 둘을 빼자
        #   `test_mapping_evidence_sweep`가 `("INSOLVENCY", "8.1")`을 새 0%로 즉시
        #   신고했다(관찰 122건). 가드가 설계대로 동작한 사례다 — 소유 신호가
        #   여럿인 taxonomy는 **전부** 대조해야 한다.
        "keywords": [
            "자본잠식",
            "부도",
            "어음부도",
...
CROSS_SIGNAL_PATTERNS = {
    "founder_fade": {
        "name": "창업주 퇴장",
        "description": (
            "창업주나 기존 대주주의 지분이 새 주식 발행으로 옅어지는 흐름입니다. "
            "경영진이 물러난 자리를 바깥 세력이 채우고, 지배권이 그쪽으로 "
            "넘어갑니다."
        ),
    ...
    "debt_spiral": {
        ...
        "description": (
            ...
            "흐름입니다. 회사가 번 돈으로 빚을 못 갚고 있다는 뜻일 수 "
            "있습니다. 다만 금리를 낮추려는 차환도 있으니 상환 재원이 "
            "무엇인지 함께 보세요."
        ),
    ...
    "reverse_split_spiral": {
        "name": "감자 나선",
        "description": (
            "주식병합이나 감자로 주가와 재무 요건을 맞춘 직후, 새 주식을 크게 "
            "찍는 일이 되풀이되는 흐름입니다. 맞춰 놓은 요건이 다시 "
            "흔들립니다."
        ),
    ...
    "related_party_hollowing": {
        "name": "특수관계 자산 공동화",
        "description": (
            "특수관계인 쪽과의 거래로 자산과 영업이 회사 밖으로 옮겨 가는 "
            "흐름입니다. 회사에 남는 자산은 줄고 값어치는 상대 쪽으로 "
            "갑니다. 정상적인 계열 거래와 겉모습이 같습니다."
        ),
    ...
    "zombie_ma": {
        "name": "무자본 M&A",
        "description": (
            "자기 돈 없이 남의 이름이나 투자조합을 내세워 회사를 사들이는 "
            "흐름입니다. 인수 뒤 전환사채를 크게 찍어 돈을 모읍니다. 이어 새 "
            "사업을 발표해 주가를 띄운 뒤 주식을 팔고 나가는 순서입니다."
        ),
    ...
    "audit_insider_dump": {
        "name": "감사의견 내부자 덤프",
        "description": (
            "감사의견이 적정으로 나오지 않을 것을 미리 안 임원·대주주가 공시 "
            "전에 주식을 파는 흐름입니다. 손실은 그 사실을 모르는 주주에게 "
            "남습니다."
        ),
    ...
    "delisting_evasion": {
        "name": "상폐 회피",
        "description": (
            "상장폐지가 다가올 때 큰 유상증자와 호재성 공시로 요건을 맞춰 넘기는 "
            "흐름입니다. 들어온 돈이 실제 돈인지, 실적이 부풀려졌는지가 "
            "갈림길입니다."
        ),
    ...
    "fake_new_biz": {
        "name": "허위 신사업 주가부양",
        "description": (
            "주업과 상관없는 유행 사업을 발표해 주가를 띄우는 흐름입니다. 계약은 "
            "형식뿐이고 상대는 서류상 회사인 경우가 있습니다. 값이 오른 사이 "
            "대주주 지분이 팔려 나갑니다."
        ),
    ...
    "capital_churn_anomaly": {
        "name": "자본 이벤트 과다 반복",
        "description": (
            "자본을 자주 건드리면서 공시를 늦게 내거나 어기는 일이 겹치는 "
            "흐름입니다. 주식 수가 어떻게 바뀌었는지 밖에서 따라가기 "
            "어려워집니다."
        ),
    ...
    "capital_backflow": {
        ...
        "description": (
            ...
            "흐름입니다. 인수에 쓴 돈을 회사에서 되찾아가는 모양이 됩니다. "
            "계열 지원과 겉모습이 같습니다."
        ),
    ...
    # `description`은 뷰어 패턴 카드의 머리 단락으로 그대로 나간다 — 개발
    # 메모는 여기 둔다.
    #  · 금감원 무자본 M&A 합동점검 집계: 조달자금 유용 경로 중 비상장주식
    #    취득이 55%로 최대(수치 인용은 `PATTERN_PROSE`가 들고 있다).
    #  · 제3자배정 유상증자 경로도 자금 이동 구조는 같은 맥락이나
    #    `signal_sequence`에는 포함하지 않았다.
    "fund_diversion_chain": {
        "name": "조달-유용 체인",
        "description": (
            "전환사채 같은 사모 방식으로 돈을 모은 뒤, 그 돈이 비상장회사 주식 "
            "매입이나 다른 회사 출자로 나가는 흐름입니다. 정상적인 사업 "
            "확장에서도 같은 순서가 나옵니다. 그래서 어디에, 얼마에, 외부 "
            "평가를 거쳐 샀는지를 원문에서 봐야 합니다."
        ),
