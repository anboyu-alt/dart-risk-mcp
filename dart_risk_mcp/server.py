...
def _fund_round_korean(tm: str | None) -> str:
    ...
    ⚠ **DART가 회차를 세 가지 모양으로 준다**(30개사 2,877건 실측,
    2026-08-27) — 숫자만 1,302 · 빈값 1,198 · `"N회차"` 197 · `"N회"` 174.
    옛 코드는 `f"제{tm}회차"`라 접미가 붙어 오는 **371건(12.9%)**이
    「제12회차**회차**」·「제9회**회차**」로 나왔다(오르비텍 골드 실측).

    각주 기호가 붙어 오는 것도 있다 — `"37회차(*)"`, 줄바꿈이 낀
    `"32회차
주1)"`(6건). 숫자만 남기되, 숫자를 못 찾으면 정제한 원문을
    그대로 보여 준다(정보를 버리지 않는다).
    """
    tm_s = " ".join((tm or "").split())
    if tm_s in _EMPTY_TM_VALUES:
        return ""
    m = re.match(r"^제?\s*(\d+)\s*(?:회차|회)?", tm_s)
    if not m:
        # 숫자가 없으면 「제…회차」로 감싸지 않는다 — 「제미상회차」처럼
        # 말이 안 되는 라벨이 된다. 원문을 그대로 보여 준다.
        return tm_s
    return f"제{m.group(1)}회차"
...
def _related_party_detail_block(
    ...
    lines = [
        "",
        "🤝 **특수관계인 자금거래 확인**",
        # 여기도 방향을 단정하고 있었다 — 같은 문장이 SIGNAL_PROSE와 이곳
        # 두 군데에 있었고, 해설만 고쳤더니 이 줄이 남았다(2026-08-24).
        "계열사·최대주주와의 자금·출자 거래입니다. 제목의 「…으로부터」는"
        " 받아온 것, 「…에 대한 출자」는 회사가 넣은 것입니다. 이자율과"
        " 규모가 조건을 따져볼 지점입니다.",
    ]
    for e, d in rows:
...
def track_debt_balance(company_name: str, year: str = "") -> str:
    ...
    if m1y_share >= 0.30:
        # ⚠ 이 비중의 분모는 **채무증권 잔액뿐**이다 — 은행 차입금·리스부채는
        # 들어 있지 않고, 회사 자산·현금과도 무관하다. 그래서 "부담이 집중"
        # 이라고만 적으면 규모가 작은 회사에서 과장이 된다(2026-08-26 실측:
        # 삼성전자는 채무증권이 6,100억뿐인데 97.6%가 1년 이내라 이 문장이
        # 붙는다 — 사실이지만 상환 부담을 뜻하지 않는다).
        #
        # 50개사 실측 분포: 잔액이 있는 43곳 중 30% 이상이 16곳(37%),
        # 중앙값 26.5% · p25 5.7% · p75 38.4%. 셋에 하나꼴로 붙는 값이라
        # 경고가 아니라 **구성 사실**로 읽히게 적는다.
        lines += [
            "",
            f"⚠ 채무증권 잔액 {total_eok:,}억원 가운데 {m1y_share:.0%}가 "
            "1년 이내 만기입니다.",
            "   이 비중의 분모는 **채무증권 잔액뿐**입니다 — 은행 차입금·"
            "리스부채는 빠져 있고 회사의 현금·자산 규모와도 무관합니다. "
            "상환 부담은 위 총 잔액의 크기와 함께 보셔야 합니다.",
        ]
...
def check_disclosure_anomaly(
    ...
    lines += [
        ...
        # 옛 문구는 "정상 기업은 보통 5% 안쪽이고, 20%를 넘으면 …"이었다.
        # 근거가 없었고 실측과 어긋난다(2026-08-24 시장 전체 44일·27,780건):
        # 같은 기간 공시 10건 이상인 **상장사 488곳**의 중앙값이 10.0%이고
        # **70%가 5%를 넘는다**. 20% 초과도 27.7%라 드문 일이 아니다.
        # ⚠ 자산운용사(E)는 펀드 상품 공시 정정이 일상이라 중앙값이 47%다 —
        #   상장사와 섞어 재면 안 된다(삼성자산운용 58.9%·KB자산운용 56.3%).
        (
            "이미 낸 공시를 고쳐서 다시 내는 비율입니다. 시장 실측(2026-07-10"
            "~08-22, 공시 27,780건)에서 같은 기간 공시 10건 이상인 상장사 "
            "488곳의 중앙값은 10%이고, 상위 25%가 23%를, 상위 10%가 39%를 "
            "넘습니다. 5%를 넘는 상장사가 70%라 비율만으로 이상 여부가 "
            "갈리지는 않으니, 같은 사안이 반복 정정되는지를 원문에서 "
            "확인하는 편이 낫습니다."
        ),
        "",
...
def track_fund_usage(company_name: str, lookback_years: int = 3) -> str:
    ...
    for rec in records:
        ...
        if _pc and _rc and not (set(_pc) & set(_rc)):
            lines.append(
                f"  용도 분류: 계획 [{'·'.join(_pc)}] → 실제 [{'·'.join(_rc)}]"
            )
        # 플래그 → 한국어 서술
        for f in rec["flags"]:
            title, body = flag_to_prose(f)
...
def track_capital_structure(
    ...
    if churn_flagged:
        ...
        # 옛 문구는 **전체** 카운트를 인용하며 "'3건 이상 몰림' 기준에 미치지
        # 못했습니다"라고 적었다. 삼성전자는 자본 이벤트 10건인데 미달이라
        # 적혀 자기모순이었다(2026-08-24). 실제 기준은 **희석성** 카운트다 —
        # 삼성전자의 10건은 전부 자사주(비희석성)라 미달이 맞다.
        summary = (
            f"🎯 최근 {lookback_years}년 동안 자본 이벤트 "
            f"{result['total_events']}건이 관찰됐습니다 — 12개월 기준 "
            f"희석성(증자·메자닌) {result['max_dilutive_12m']}건 · "
            f"비희석성(자사주 등) {result['max_non_dilutive_12m']}건. "
            "집중 판정은 희석성 3건 이상이거나 희석성·비희석성이 각각 2건 "
            "이상일 때 붙는데, 여기에는 해당하지 않습니다. "
