...
⚠ **2026-09-11 재설계.** 접는 단위가 (조달유형, 회차, 납입일, 계획용도,
계획금액)이었는데 계획 용도·금액을 제출사가 보고서마다 다르게 채워(실측 CSA
코스믹: 같은 회차의 용도 칸이 `"24,999"` ↔ `"운영자금"`) **한 건도 접히지
않는** 회사가 나왔다. 이제 **조달건**(조달유형, 납입일, 회차)으로 묶고 그
그룹에서 **가장 최신 보고서 스냅샷**(사업연도 + 보고서코드)의 행을 **전부**
남긴다 — 한 스냅샷 안의 행은 서로 다른 사실이라 뭉개지 않는다.
상세는 `tests/test_fund_snapshot_report_code.py`.
...
def test_연도가_비정상이어도_죽지_않는다():
    """연도를 못 읽으면 **어느 쪽이 최신인지 모른다** — 추정하지 않는다.

    2026-09-11 이전에는 하나로 접혔다. 그건 「둘 중 아무거나 대표로 삼는다」와
    같았고, 지금은 같은(=알 수 없는) 스냅샷의 행으로 보아 둘 다 남긴다.
    실제 경로에서는 `year`가 연도 루프의 int라 이 분기에 오지 않는다.
    """
