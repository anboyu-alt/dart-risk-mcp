...
class TestAuthGate(unittest.TestCase):
    def test_missing_token_is_401(self):
        resp = handle(_req("POST", "/api/se/analyze", {"company": "셀트리온"}, token=""),
...
class TestCreate(unittest.TestCase):
    def test_creates_job(self):
        ...
        with mock.patch("se_server.api.handlers.resolve_corp",
            ...
            resp = handle(_req("POST", "/api/se/analyze",
    ...
    def test_missing_company_is_400(self):
        resp = handle(_req("POST", "/api/se/analyze", {}), _deps())
        self.assertEqual(resp.status, 400)

    def test_missing_dart_key_is_400(self):
        resp = handle(_req("POST", "/api/se/analyze", {"company": "셀트리온"}, dart_key=""),
                      _deps())
        self.assertEqual(resp.status, 400)
    ...
    def test_unknown_company_is_404(self):
        ...
        with mock.patch("se_server.api.handlers.resolve_corp", return_value=None):
            resp = handle(_req("POST", "/api/se/analyze", {"company": "없는회사"}), _deps())
...
class TestStep(unittest.TestCase):
    def _make_job(self, store, user_id="user-1"):
        with mock.patch("se_server.api.handlers.resolve_corp",
                        return_value=("회사", {"corp_code": "0"})):
            resp = handle(_req("POST", "/api/se/analyze", {"company": "회사"}),
    ...
    def test_runs_a_step(self):
        ...
        with mock.patch("se_server.jobs.runner.resolve_callable",
            ...
            resp = handle(_req("POST", f"/api/se/analyze/{job_id}/step"), _deps(store=store))
    ...
    def test_other_user_gets_404(self):
        ...
        resp = handle(_req("POST", f"/api/se/analyze/{job_id}/step"),
                      Deps(store=store, auth=_Auth(user_id="침입자")))
        self.assertEqual(resp.status, 404)

    def test_unknown_job_is_404(self):
        resp = handle(_req("POST", "/api/se/analyze/없는작업/step"), _deps())
        self.assertEqual(resp.status, 404)

    def test_missing_dart_key_is_400(self):
        store = MemoryJobStore()
        job_id = self._make_job(store)
        resp = handle(_req("POST", f"/api/se/analyze/{job_id}/step", dart_key=""),
                      _deps(store=store))
        self.assertEqual(resp.status, 400)
    ...
    def test_empty_user_id_is_rejected(self):
        for bad in ("", None, 123):
            with self.subTest(user_id=bad):
                resp = handle(_req("GET", "/api/se/analyze/j1"),
    ...
    def test_non_dict_body_is_400_not_500(self):
        for body in (None, ["x"], "문자열", 42):
            with self.subTest(body=body):
                req = Request("POST", "/api/se/analyze",
...
class TestStepOwnershipIsPassedThrough(unittest.TestCase):
    ...
    def test_run_step_receives_user_id(self):
        ...
        with mock.patch("se_server.api.handlers.resolve_corp",
                        return_value=("회사", {"corp_code": "0"})):
            created = handle(_req("POST", "/api/se/analyze", {"company": "회사"}),
        ...
        with mock.patch.object(runner, "run_step", side_effect=spy), \
            ...
            handle(_req("POST", f"/api/se/analyze/{job_id}/step"),
                   Deps(store=store, auth=_Auth(user_id="침입자")))
...
class TestGet(unittest.TestCase):
    def test_returns_progress(self):
        ...
        with mock.patch("se_server.api.handlers.resolve_corp",
            ...
            created = handle(_req("POST", "/api/se/analyze", {"company": "회사"}),
                             _deps(store=store))
        resp = handle(_req("GET", f"/api/se/analyze/{created.body['job_id']}"),
                      _deps(store=store))
        self.assertEqual(resp.status, 200)
...
class TestSecrets(unittest.TestCase):
    def test_dart_key_never_appears_in_response(self):
        ...
        with mock.patch("se_server.api.handlers.resolve_corp",
                        return_value=("회사", {"corp_code": "0"})):
            resp = handle(Request("POST", "/api/se/analyze",
