"""리픽싱 표기 변형을 `CB_BW`가 잡는지 — 그리고 **행사는 안 잡는지** 고정한다.
...
**735건 / 304개사** 무신호로 나왔다. 넣고 싶어지는 자리지만 **taxonomy가 없다** —
`1.1`은 조정이고, 카테고리 1·2를 전수 확인해도 「행사(실제 희석이 일어난 시점)」를
담는 항목이 없다(`1.4` RCPS도 발행 조건이지 행사가 아니다).

뜻이 어긋나는 자리에 밀어 넣는 것이 `INQUIRY` 실사고(v1.12.1 — 정례 매매정지를
삼켜 패턴 카드 3개 오발화)의 뿌리였고, v1.13.4·v1.13.5에서 후보 7종 중 5종이
같은 이유로 기각됐다. 새 taxonomy가 필요한 건이라 위험 목록 10번에 남겼다.

**이 테스트가 실패하면** 누군가 행사 계열을 넣은 것이다 — 위험 목록 10번과
이 파일을 함께 고쳐라.
...
@pytest.mark.parametrize("title", [
    # ⚠ 「행사」 계열 — 담을 taxonomy가 없어 **일부러 넣지 않았다**
    "전환청구권행사",
    "전환청구권행사 (제1회차)",
    "전환청구권ㆍ신주인수권ㆍ교환청구권행사",
    "전환주식의전환청구권행사",
...
def test_행사는_아직_잡지_않는다(title):
    """넣으려면 새 taxonomy가 먼저다 — 위험 목록 10번 B를 함께 고쳐라."""
    assert not match_signals(title), (
        f"행사 계열에 신호가 붙기 시작했다: {title} → "
        f"{[m['key'] for m in match_signals(title)]}\n"
        "taxonomy가 맞는지 먼저 확인하고, 위험 목록 10번과 이 테스트를 함께 갱신하라.")
