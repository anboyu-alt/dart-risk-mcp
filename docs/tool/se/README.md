...
로그인 후(`#main`) 상단 진행률 바(`#bar`) 아래에 회사 입력 폼이 있습니다.

1. **회사명 또는 종목코드**를 입력합니다(`#company-input`).
2. **조회 범위**를 고릅니다(`#lookback-years`, 1~5년 — 서버 계약
   `se_server/api/handlers.py`의 `_MIN_YEARS`/`_MAX_YEARS`와 동일합니다).
   기본값은 1년입니다 — 다년 조회는 훨씬 오래 걸립니다.
3. **분석 시작** 버튼을 누르거나 입력창에서 Enter를 칩니다
   (`#analyze-btn` 클릭·`#company-input` Enter 모두 `doAnalyze()` →
   `analyze(company, lookbackYears)`로 이어집니다).

분석은 수 분이 걸릴 수 있습니다. 진행 중에는 버튼이 비활성화되고 재클릭이
무시됩니다(`ANALYZING` 가드) — 연타로 같은 작업이 중복 생성돼 DART 호출
한도를 태우는 것을 막기 위해서입니다. 새 회사를 조회하면 이전 회사의
섹션·오류·패널은 화면에서 모두 지워진 뒤 새로 채워집니다
(`renderHeadPlaceholder`가 `#body`를 비웁니다 — 두 회사의 정보가 섞여
보이지 않도록).

탭을 닫았다 다시 열거나 재로그인하면, 12시간 이내에 시작한 미완료 작업을
자동으로 이어받습니다(`resumeIfAny()`) — 다만 재개가 실패해도 화면은
정상적으로 사용 가능합니다(재개는 편의 기능이지 필수 경로가 아닙니다).
...
| 로그인 후 회사 입력·조회 시작 | 진행률 바 아래 회사명 입력창·조회 범위 선택·"분석 시작" 버튼이 보인다. 회사명을 입력해 시작하면(또는 Enter) 헤더가 먼저 뜨고 섹션이 위에서부터 채워진다 |
| 분석 진행 중 재클릭 | 버튼이 비활성화되고 새 작업이 중복 생성되지 않는다 |
| 새 회사로 다시 조회 | 이전 회사의 섹션·오류가 화면에서 사라지고 새 회사 결과만 보인다 |
