...
**⚠️ 정정(2026-07-28):** 이 표는 원래 `수익성`·`안정성`처럼 **접미사 없는** 이름으로 적혀 있었다. 그건 실측이 아니라 계획 작성자가 탐색 스크립트에서 **자기가 만든 라벨을 출력해놓고** 실측으로 옮겨 적은 것이다. DART가 실제로 주는 값은 **`지표` 접미사가 붙는다.** 이 오류 때문에 Task 2의 분류표가 라이브 데이터에서 한 건도 매칭되지 않아 화면이 통째로 비는 결함이 생겼고(테스트는 전부 초록이었다), Task 3에서 발견해 고쳤다.
