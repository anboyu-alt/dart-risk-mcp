...
| `dart_risk_mcp/core/taxonomy.py` | taxonomy·복합 패턴·페이즈 매핑 | 수정: `TAXONOMY["2.5"]`, `CROSS_SIGNAL_PATTERNS` 1개, `_PHASE_MAP` 5개 키 추가 |
...
class TestV6Taxonomy(unittest.TestCase):
    def test_2_5_exists(self):
        self.assertIn("2.5", TAXONOMY)
        entry = TAXONOMY["2.5"]
...
git commit -m "feat(taxonomy): add 2.5 capital churn + capital_churn_anomaly pattern"
```
...
- **신규 신호 키 5개**: `CAPITAL_CHURN`(2.5), `AR_SURGE`(6.1), `INVENTORY_SURGE`(6.1), `CASH_GAP`(6.1), `CAPITAL_IMPAIRMENT`(8.2)
- **신규 taxonomy 2.5** (Category 2 자본구조) — 자본 이벤트 과다 반복
- **신규 복합 패턴 1개**: `capital_churn_anomaly` (2.5 + 4.3)
