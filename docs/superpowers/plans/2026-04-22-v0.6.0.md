...
class TestV6Taxonomy(unittest.TestCase):
    ...
    def test_capital_churn_pattern_registered(self):
        # CROSS_SIGNAL_PATTERNS는 dict[str, dict] 구조 (key=패턴명)
...
기존 siblings(`TAXONOMY["2.6"]` 등) 스키마를 그대로 따른다(`id`·`category`·`name`·`base_score`·`severity`·`keywords`·`red_flags`·`field_evidence`·`investor_implication`·`crisis_timeline_months`).
...
`CROSS_SIGNAL_PATTERNS`(dict[str, dict] — key=패턴명, value=상세)에 항목 추가:

```python
    "capital_churn_anomaly": {
        "name": "capital_churn_anomaly",
        "description": "잦은 자본 변동과 공시 지연·위반이 겹치면 자본 투명성 훼손 우려",
        "signal_sequence": ["2.7", "4.3"],  # siblings는 signal_sequence 사용
        "timeline_months": 12,
        "severity": "HIGH",
        "field_evidence": [
