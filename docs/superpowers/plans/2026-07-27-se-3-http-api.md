...
| `vercel.json` | **수정** — `/api/se/*`만 rewrite. 릴레이의 단일 세그먼트 정규식과 구조적으로 겹치지 않는다 |
...
class TestMatch(unittest.TestCase):
    ...
    def test_get_job(self):
        self.assertEqual(match("GET", "/api/se/analyze/abc123"), ("get", {"job_id": "abc123"}))

    def test_trailing_slash_is_tolerated(self):
        self.assertEqual(match("POST", "/api/se/analyze/"), ("create", {}))

    def test_query_string_is_ignored(self):
        self.assertEqual(match("GET", "/api/se/analyze/abc?x=1"), ("get", {"job_id": "abc"}))
...
# 경로에 /se/ 세그먼트를 둔다. 기존 CORS 릴레이(api/[endpoint].js)가
# ^/api/[^/]+$ 를 차지하므로 /api/se/analyze는 같은 세그먼트를 다투게 되고,
# rewrite와 동적 라우트 중 무엇이 이기는지가 Vercel 내부 우선순위에 달린다.
# 릴레이로 새면 403이 나는데 하위 경로(/api/se/analyze/{id}/step)는 경쟁자가
# 없어 정상 동작하는 **비대칭 실패**라 원인 파악이 매우 어렵다.
# 두 세그먼트면 릴레이 정규식이 구조적으로 매칭 불가라 우선순위에 의존하지 않는다.
...
def match(method: str, path: str) -> tuple[str, dict] | None:
    ...
    for route_method, pattern, name in _ROUTES:
        ...
        # 직전**에서도 매치되므로, `match` + `$`면 "/api/se/analyze/abc
"이
...
| 라우트 | 요청 | 응답 |
|---|---|---|
| `POST /api/se/analyze` | `{"company": "셀트리온", "lookback_years": 1}` + `X-DART-Key` | `201 {"job_id", "company", "total"}` |
| `POST /api/se/analyze/{id}/step` | `X-DART-Key` | `200 {"done", "processed", "finished", "total", "stalled"}` |
| `GET /api/se/analyze/{id}` | — | `200 {"job_id", "company", "status", "finished", "total", "sections"}` |
...
`vercel.json` — **기존 파일을 덮어쓰지 말고 `rewrites`만 추가한다.**

저장소에는 이미 `api/[endpoint].js`(DART CORS 릴레이)가 있고 **공개 뷰어(`docs/tool/index.html`)가 이를 사용 중**이다. `/api/(.*)`로 전부 rewrite하면 `/api/list.json`·`/api/document.xml` 같은 릴레이 호출이 전부 Python 핸들러로 빨려 들어가 **라이브 공개 뷰어가 깨진다.**

rewrite 범위를 `/api/se/` 이하로만 좁히고, 기존 필드(`regions`·`outputDirectory` 등)는 전부 보존한다.

**왜 `/api/se/analyze`가 아니라 `/api/se/analyze`인가:** 릴레이는 `^/api/[^/]+$`(단일 세그먼트)를 차지한다. `/api/se/analyze`는 그 정규식에 **매칭되므로** rewrite와 동적 라우트 중 무엇이 이기는지가 Vercel 내부 우선순위에 달린다. 릴레이로 새면 `403 {"error":"forbidden"}`이 나오는데, 하위 경로(`/api/se/analyze/{id}/step`)는 경쟁자가 없어 정상 동작한다 — 이 **비대칭 실패**는 원인 파악이 매우 어렵다. 두 세그먼트면 구조적으로 겹치지 않는다.
...
```json
{
  ...
  "installCommand": null,
  "regions": ["icn1"],
  "outputDirectory": "docs/tool",
  "trailingSlash": false,
  "rewrites": [
    { "source": "/api/se/(.*)", "destination": "/api/index" }
  ]
}
```

rewrite에 걸리지 않는 `/api/*`는 Vercel의 파일 기반 라우팅으로 `api/[endpoint].js`에 그대로 도달한다.

> `functions.maxDuration`은 설정하지 않는다. 플랜별 상한이 달라 **배포 후 실측해 정한다**(설계 §11 미결정 사항). 상한을 넘기면 함수가 항목 도중 죽어 진행이 유실된다.

> `api/`에 `__init__.py`를 추가하지 않는다. 네임스페이스 패키지로 import가 되고, Vercel의 파일 기반 함수 탐지(`api/*.py`)를 교란할 위험만 있다.
...
- **공개 뷰어 회귀 확인** — 배포 후 `dart-risk-mcp.vercel.app`에서 종목 조회가 여전히 되는지 확인한다. `/api/list.json`·`/api/exctvSttus.json`이 릴레이로 그대로 도달해야 한다
- **rewrite 경로 전달 확인** — 프리뷰에서 `POST /api/se/analyze`가 401(핸들러 도달)을 주는지 확인한다. 403이면 릴레이로 샌 것이고, 모든 경로가 404면 rewrite 후 `self.path`가 원본 경로로 전달되지 않는 것이다
- **`maxDuration` 실측 후 설정** — Hobby 기본 상한(10초)은 `_DEFAULT_BUDGET`(25초)보다 작아, 그대로 두면 `run_step`이 항목 도중 죽어 진행이 유실된다. `_DEFAULT_BUDGET`은 `OVERSIZED_RESERVE`(20)보다 커야 하므로 25초가 사실상 하한이다 — **상한이 25초 미만인 플랜에서는 다건 조회 항목을 실행할 수 없다**
- **CORS 정책 결정** — 현재 어댑터는 `OPTIONS`에 501을 돌려준다(`BaseHTTPRequestHandler` 기본). SE-4 프런트를 **같은 Vercel 배포에 두면 동일 출처라 프리플라이트가 없어 문제되지 않는다.** 다른 출처에서 호출할 계획이면 `do_OPTIONS` + 출처 allowlist가 필요하다 — 인증 API이므로 릴레이처럼 `*`를 쓰면 안 된다
