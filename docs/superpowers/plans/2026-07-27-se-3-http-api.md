...
| `vercel.json` | **수정** — `/api/analyze*`만 rewrite. 기존 CORS 릴레이 경로는 건드리지 않는다 |
...
rewrite 범위를 `/api/analyze` 이하로만 좁히고, 기존 필드(`regions`·`outputDirectory` 등)는 전부 보존한다:
...
```json
{
  ...
  "trailingSlash": false,
  "rewrites": [
    { "source": "/api/analyze", "destination": "/api/index" },
    { "source": "/api/analyze/(.*)", "destination": "/api/index" }
  ]
}
```
...
- **공개 뷰어 회귀 확인** — 배포 후 `dart-risk-mcp.vercel.app`에서 종목 조회가 여전히 되는지 확인한다. `/api/analyze*` rewrite가 기존 CORS 릴레이(`api/[endpoint].js`)를 삼키지 않아야 한다
