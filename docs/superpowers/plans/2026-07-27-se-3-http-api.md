...
# ^/api/[^/]+$ 를 차지하므로 /api/analyze는 같은 세그먼트를 다투게 되고,
# rewrite와 동적 라우트 중 무엇이 이기는지가 Vercel 내부 우선순위에 달린다.
# 릴레이로 새면 403이 나는데 하위 경로(/api/analyze/{id}/step)는 경쟁자가
...
**왜 `/api/analyze`가 아니라 `/api/se/analyze`인가:** 릴레이는 `^/api/[^/]+$`(단일 세그먼트)를 차지한다. `/api/analyze`는 그 정규식에 **매칭되므로** rewrite와 동적 라우트 중 무엇이 이기는지가 Vercel 내부 우선순위에 달린다. 릴레이로 새면 `403 {"error":"forbidden"}`이 나오는데, 하위 경로(`/api/analyze/{id}/step`)는 경쟁자가 없어 정상 동작한다 — 이 **비대칭 실패**는 원인 파악이 매우 어렵다. 두 세그먼트면 구조적으로 겹치지 않는다.
