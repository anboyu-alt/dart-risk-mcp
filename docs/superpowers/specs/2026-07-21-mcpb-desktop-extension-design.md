...
- `extension/pyproject.toml` — 의존성으로 **PyPI의 `dart-risk-mcp==<버전>`을 정확히 고정**(구현 중 확정: 소스 번들 복제 대신 배포 버전 의존 → 소스 드리프트 0, 번들 초소형). `mcp`·`requests`는 그 패키지가 전이 의존으로 끌어옴. `[tool.uv] package = false`로 런처 자체는 빌드하지 않음.
- `extension/src/server.py` — 3줄 진입점(`from dart_risk_mcp.server import main; main()`). uv가 pyproject로 격리 환경을 만들어 실행.
