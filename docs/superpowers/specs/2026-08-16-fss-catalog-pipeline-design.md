...
| # | 리스크 | 대응 |
|---|---|---|
| 0 | **FSS 개인 인증키의 조회기간·호출 한도** (2026-08-16 사용자 제보, 실호출 미검증): 조회기간 상한 **31일**, **일일 호출 30회**로 알려져 있다. 사실이면 2010~2026 백필(약 200개 월 청크)은 한 번에 끝나지 않고 **최소 7일**에 걸쳐 나눠 받아야 한다 | 월 단위 청크는 31일 상한과 이미 정합적(테스트로 고정). 일일 한도는 `--max-calls`(기본 25)로 한 실행의 호출을 제한하고, 청크마다 JSONL append + `collect_state.json`에 완료 청크를 기록해 `--resume`으로 이어받는다. 중단돼도 그때까지의 수집분이 남는다. **키 확보 후 실제 한도를 실측해 이 항목을 갱신할 것** |
